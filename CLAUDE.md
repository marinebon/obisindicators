...
- `occ_h3` — **species-level** counts at resolution tiers 3/5/7
  (`H3T_RES_TIERS`), clustered by `(res, taxonomy)` for zonemap pruning;
  used for live taxon/year-filtered queries.
...
years → `idx_h3_taxon`; an `aphiaid` filter → live `occ_h3` filtered to
the WoRMS subtree (see below); anything else
finer/multi-value/year-ranged → live aggregation over `occ_h3`.
