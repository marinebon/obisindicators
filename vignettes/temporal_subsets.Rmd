...
grid_1960s <- dgcellstogrid(dggs, metrics_1960s$cell) %>% 
  ...
  left_join(
    metrics_1960s,
    by = "cell")
grid_1970s <- dgcellstogrid(dggs, metrics_1960s$cell) %>% 
  st_wrap_dateline() %>% 
  rename(cell = seqnum) %>% 
  left_join(
    metrics_1960s,
    by = "cell")
grid_1980s <- dgcellstogrid(dggs, metrics_1960s$cell) %>% 
  st_wrap_dateline() %>% 
  rename(cell = seqnum) %>% 
  left_join(
    metrics_1960s,
    by = "cell")
grid_1990s <- dgcellstogrid(dggs, metrics_1960s$cell) %>% 
  st_wrap_dateline() %>% 
  rename(cell = seqnum) %>% 
  left_join(
    metrics_1960s,
    by = "cell")
grid_2000s <- dgcellstogrid(dggs, metrics_1960s$cell) %>% 
  st_wrap_dateline() %>% 
  rename(cell = seqnum) %>% 
  left_join(
    metrics_1960s,
    by = "cell")
grid_2010s <- dgcellstogrid(dggs, metrics_1960s$cell) %>% 
  st_wrap_dateline() %>% 
  rename(cell = seqnum) %>% 
  left_join(
    metrics_1960s,
